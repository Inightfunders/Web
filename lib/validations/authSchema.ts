...
/*    if (data.role === "partner") {
      ...
      if (!data.confirmPassword) {
        ctx.addIssue({
          ...
          message: "Confirm password is required for partners",
...
export const personalDetailsSchema = z.object({
  ...
  dateOfBirth: z
    ...
    .min(10, "Date of Birth must be in the format MM/DD/YYYY")
    .refine(
      (value) => {
        const regex =
  ...
  ssn: z
    ...
    .min(11, "SSN must be in the format XXX-XX-XXXX")
    .max(11, "SSN must be in the format XXX-XX-XXXX"),
});
