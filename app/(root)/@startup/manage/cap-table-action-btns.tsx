...
export default function CapTableActionBtns({ document_link, capTableId }: { document_link: string, capTableId: number }) 
{
    ...
    const handleUploadNewDocument = async (e: ChangeEvent<HTMLInputElement>) => {
        ...
        await supabase.from('cap_tables').update({ document_link: publicUrl }).eq('id', capTableId!)
    ...
    const handleDelete = async () => {
        ...
        await supabase.from('cap_tables').delete().eq('id', capTableId!)
        await supabase.storage
            .from('capTables')
            .remove([fileName!])
