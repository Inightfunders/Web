...
export default function FinancialStatementsActionBtns({ document_link, financialStatementsId }: { document_link: string, financialStatementsId: number }) 
{
    ...
    const handleUploadNewDocument = async (e: ChangeEvent<HTMLInputElement>) => {
        ...
        await supabase.from('financial_statements').update({ document_link: publicUrl }).eq('id', financialStatementsId!)
    ...
    const handleDelete = async () => {
        ...
        await supabase.from('financial_statements').delete().eq('id', financialStatementsId!)
        await supabase.storage
            .from('financialStatements')
            .remove([fileName!])
