...
import { ndaSchema } from "@/lib/validations/startupsSchema"
import { z } from "zod"
import {
    Form,
    FormControl,
...
import { createNda } from "@/lib/actions/startup"

export default function UploadNdaDocuments()
{
    ...
    const form = useForm<z.infer<typeof ndaSchema>>({
    ...
    const onSubmit = async (values: z.infer<typeof ndaSchema>) => {
        setIsLoading(true)
        ...
        const { error: storageError } = await supabase.storage.from('nda').upload(fileName, values.document)
        ...
        const { data: { publicUrl } } = supabase.storage.from('nda').getPublicUrl(fileName)

        const result = await createNda({ name: values.name, document_link: publicUrl})
