...
export default function FinancialProjectionActionBtns({ document_link, FinancialProjectionId }: { document_link: string, FinancialProjectionId: number }) 
{
    ...
    const downloadTermSheet = async () => {
        const { data } = await supabase.storage.from('financialProjection').download(fileName!)
    ...
    const handleUploadNewDocument = async (e: ChangeEvent<HTMLInputElement>) => {
        ...
        const { data: { publicUrl } } = supabase.storage.from('financialProjection').getPublicUrl(newFileName)

        await supabase.storage.from('financialProjection').remove([fileName!])

        await supabase.from('financial_projection').update({ document_link: publicUrl }).eq('id', FinancialProjectionId!)
    ...
    const handleDelete = async () => {
        ...
        await supabase.from('financial_projection').delete().eq('id', FinancialProjectionId!)
        await supabase.storage.from('financialProjection').remove([fileName!])
