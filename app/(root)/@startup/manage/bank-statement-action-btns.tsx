...
export default function BankStatementActionBtns({ document_link, bankStatementId }: { document_link: string, bankStatementId: number })
{
    ...
    const downloadTermSheet = async () => {
        const { data } = await supabase.storage.from('bankStatements').download(fileName!)
    ...
    const handleUploadNewDocument = async (e: ChangeEvent<HTMLInputElement>) => {
        ...
        const { error: storageError } = await supabase.storage.from('bankStatements').upload(newFileName, uploadedFile)
        ...
        const { data: { publicUrl } } = supabase.storage.from('bankStatements').getPublicUrl(newFileName)

        await supabase.storage.from('bankStatements').remove([fileName!])

        await supabase.from('bank_statements').update({ document_link: publicUrl }).eq('id', bankStatementId!)
    ...
    const handleDelete = async () => {
        ...
        await supabase.from('bank_statements').delete().eq('id', bankStatementId!)
        await supabase.storage.from('bankStatements').remove([fileName!])
