...
export default function OtherDocumentsActionBtns({ document_link, otherDocumentsId }: { document_link: string, otherDocumentsId: number }) 
{
    ...
    const downloadTermSheet = async () => {
        const { data } = await supabase.storage.from('otherDocuments').download(fileName!)
    ...
    const handleUploadNewDocument = async (e: ChangeEvent<HTMLInputElement>) => {
        ...
        const { data: { publicUrl } } = supabase.storage.from('otherDocuments').getPublicUrl(newFileName)

        await supabase.storage.from('otherDocuments').remove([fileName!])

        await supabase.from('other_documents').update({ document_link: publicUrl }).eq('id', otherDocumentsId!)
    ...
    const handleDelete = async () => {
        ...
        await supabase.from('other_documents').delete().eq('id', otherDocumentsId!)
        await supabase.storage.from('otherDocuments').remove([fileName!])
