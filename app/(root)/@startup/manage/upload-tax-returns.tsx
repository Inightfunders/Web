...
import { taxReturnsSchema } from "@/lib/validations/startupsSchema";
import { z } from "zod";
import {
  Form,
  FormControl,
...
import { createTaxReturns } from "@/lib/actions/startup";

export default function UploadTaxReturns() {
  ...
  const form = useForm<z.infer<typeof taxReturnsSchema>>({
    resolver: zodResolver(taxReturnsSchema),
    defaultValues: {
      name: "",
      document: new File([], ""),
    },
  ...
  const onSubmit = async (values: z.infer<typeof taxReturnsSchema>) => {
    setIsLoading(true);
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from("taxReturns").getPublicUrl(fileName);

    const result = await createTaxReturns({
      name: values.name,
  ...
  return (
    <>
      <button
        ...
        className="w-40 h-10 bg-[#FF7A00] text-white text-sm font-semibold rounded-[4px] flex items-center justify-center"
      >
        Upload document
      </button>
            ...
            >
              ...
              <DialogFooter>
                ...
                <Button
                  disabled={isLoading}
                  type="submit"
                  className="bg-[#FF7A00] text-white rounded-[4px] flex items-center justify-center gap-2"
                >
