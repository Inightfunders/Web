...
import { bankStatementsSchema } from "@/lib/validations/startupsSchema"
import { z } from "zod"
import {
    Form,
    FormControl,
...
import { createBankStatements } from "@/lib/actions/startup"

export default function UploaBankStatementDocuments()
{
    ...
    const form = useForm<z.infer<typeof bankStatementsSchema>>({
        resolver: zodResolver(bankStatementsSchema),
        defaultValues: {
            name: '',
            document: new File([], "")
        },
    ...
    const onSubmit = async (values: z.infer<typeof bankStatementsSchema>) => {
        setIsLoading(true)
        ...
        const { error: storageError } = await supabase.storage.from('bankStatements').upload(fileName, values.document)
        ...
        const { data: { publicUrl } } = supabase.storage.from('bankStatements').getPublicUrl(fileName)

        const result = await createBankStatements({ name: values.name, document_link: publicUrl})
