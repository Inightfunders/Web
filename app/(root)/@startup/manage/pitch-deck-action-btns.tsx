...
export default function PitchDeckActionBtns({ document_link, pitchDeckId }: { document_link: string, pitchDeckId: number }) 
{
    ...
    const handleUploadNewDocument = async (e: ChangeEvent<HTMLInputElement>) => {
        ...
        await supabase.from('pitch_decks').update({ document_link: publicUrl }).eq('id', pitchDeckId!)
    ...
    const handleDelete = async () => {
        ...
        await supabase.from('pitch_decks').delete().eq('id', pitchDeckId!)
        await supabase.storage
            .from('pitchDecks')
            .remove([fileName!])
