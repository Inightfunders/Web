...
export default function NdaDocumentsActionBtns({ document_link, ndaId }: { document_link: string, ndaId: number }) 
{
    ...
    const downloadTermSheet = async () => {
        const { data } = await supabase.storage.from('nda').download(fileName!)
    ...
    const handleUploadNewDocument = async (e: ChangeEvent<HTMLInputElement>) => {
        ...
        const { error: storageError } = await supabase.storage.from('nda').upload(newFileName, uploadedFile)
        ...
        const { data: { publicUrl } } = supabase.storage.from('nda').getPublicUrl(newFileName)

        await supabase.storage.from('nda').remove([fileName!])

        await supabase.from('nda').update({ document_link: publicUrl }).eq('id', ndaId!)
    ...
    const handleDelete = async () => {
        ...
        await supabase.from('nda').delete().eq('id', ndaId!)
        await supabase.storage.from('nda').remove([fileName!])
