...
import { financialStatementsSchema } from "@/lib/validations/startupsSchema"
import { z } from "zod"
import {
    Form,
    FormControl,
...
import { createFinancialStatements } from "@/lib/actions/startup"

export default function UploadFinancialStatements()
{
    ...
    const form = useForm<z.infer<typeof financialStatementsSchema>>({
        resolver: zodResolver(financialStatementsSchema),
        defaultValues: {
            name: '',
            document: new File([], "")
        },
    ...
    const onSubmit = async (values: z.infer<typeof financialStatementsSchema>) => {
        setIsLoading(true)
        ...
        const result = await createFinancialStatements({ name: values.name, document_link: publicUrl})
