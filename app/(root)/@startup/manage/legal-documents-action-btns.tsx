...
export default function LegalDocumentsActionBtns({ document_link, legalDocumentsId }: { document_link: string, legalDocumentsId: number }) 
{
    ...
    const handleUploadNewDocument = async (e: ChangeEvent<HTMLInputElement>) => {
        ...
        await supabase.from('legal_documents').update({ document_link: publicUrl }).eq('id', legalDocumentsId!)
    ...
    const handleDelete = async () => {
        ...
        await supabase.from('legal_documents').delete().eq('id', legalDocumentsId!)
        await supabase.storage
            .from('legalDocuments')
            .remove([fileName!])
