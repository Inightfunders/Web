...
import { pitchDeckSchema } from "@/lib/validations/startupsSchema";
import { z } from "zod";
import {
  Form,
  FormControl,
...
import { createPitchDeck } from "@/lib/actions/startup";
...
export default function UploadPitchDeck() {
  ...
  const form = useForm<z.infer<typeof pitchDeckSchema>>({
  ...
  const onSubmit = async (values: z.infer<typeof pitchDeckSchema>) => {
    setIsLoading(true);
    ...
    const {
      data: { publicUrl },
    } = supabase.storage.from("pitchDecks").getPublicUrl(fileName);
