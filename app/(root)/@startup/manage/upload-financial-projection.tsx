...
import { financialProjectionSchema } from "@/lib/validations/startupsSchema"
import { z } from "zod"
import {
    Form,
    FormControl,
...
import { createFinancialProjection } from "@/lib/actions/startup"

export default function UploadFinancialProjection()
{
    ...
    const form = useForm<z.infer<typeof financialProjectionSchema>>({
        resolver: zodResolver(financialProjectionSchema),
        defaultValues: {
            name: '',
            document: new File([], "")
        },
    ...
    const onSubmit = async (values: z.infer<typeof financialProjectionSchema>) => {
        setIsLoading(true)
        ...
        const { data: { publicUrl } } = supabase.storage.from('financialProjection').getPublicUrl(fileName)

        const result = await createFinancialProjection({ name: values.name, document_link: publicUrl})
            ...
            <Dialog open={open} onOpenChange={setOpen}>
                <DialogContent className='bg-[#1A1A1A] rounded-xl border-none space-y-4'>
                    <DialogHeader className='font-semibold text-xl text-white'>
                        Upload a financial projection document
                    </DialogHeader>
                    <Form {...form}>
