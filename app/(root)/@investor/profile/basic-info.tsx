...
export default function BasicInfo() {
  ...
  const handleUploadNewImage = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const { data: uploadData, error: uploadError } = await supabase.storage.from("profileImg")
    ...
    const { data } = supabase.storage.from("profileImg").getPublicUrl(`profile/${newFileName}`);
    ...
    const { error: updateError } = await supabase.from("users").update({ profile_img: publicUrl }).eq("id", userId);
  ...
  const handleDeleteImage = async () => {
    ...
    const { error: deleteError } = await supabase.storage.from("profileImg").remove([`profile/${fileName}`]);
    ...
    const { error: updateError } = await supabase.from("users").update({ profile_img: null }).eq("id", user?.user?.id);
