        ...
        <div className="flex flex-col items-center justify-center gap-6">
          ...
          <div className="mt-4 flex flex-col sm:flex-row items-center gap-4">
            <input
              ...
              className="bg-red border-black text-black w-[294px] outline py-1.5"
              value={signature}
              onChange={(e) => setSignature(e.target.value)}
            />
            {signatureError && (
              <p className="text-red-500 text-left text-[10px]">
                {signatureError}
              </p>
            )}
            <button
              onMouseDown={handleSubmitSignature}
              className="w-fit rounded-[5px] bg-black text-white !text-[10px] mt-4 sm:ml-auto text-xs px-2.5 text-nowrap py-1.5"
            >
              Proceed
            </button>
          </div>
