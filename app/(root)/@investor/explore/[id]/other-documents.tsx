...
import { getOthersDocuments } from "@/lib/actions/investor";
import {
  Table,
  TableBody,
  TableCell,
...
export default function OtherDocuments({ investorId, startupId }: Props) {
    ...
    useEffect(() => {
          const fetchOtherDocument = async () => {
          try {
              const data = await getOthersDocuments(investorId, startupId);
    ...
    <section className="flex w-full flex-col gap-4">
      <div className="flex items-center justify-between gap-4">
        <Table className="bg-white">
            ...
            <TableBody>
              {otherDocument.length > 0 ? otherDocument?.map((item, index) => (
