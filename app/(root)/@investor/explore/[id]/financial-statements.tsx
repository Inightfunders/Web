...
import { getFinancialStatements, getFinancialProjection, getBankStatements } from "@/lib/actions/investor";
import {
  Table,
  TableBody,
  TableCell,
...
export default function FinancialStatements({ investorId, startupId }: Props) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [fsData, fpData, bsData] = await Promise.all([
                    getFinancialStatements(investorId, startupId),
                    getFinancialProjection(investorId, startupId),
                    getBankStatements(investorId, startupId)
                ]);

                setFinancialStatement(fsData ?? []);
                setFinancialProjection(fpData ?? []);
                setBankStatements(bsData ?? []);
            } catch (error) {
    ...
    return (
        <div>
            <TableSection title="Financial Statements" data={financialStatement} handleView={handleView} />
            
            <TableSection title="Financial Projection" data={financialProjection} handleView={handleView} />
            
            <TableSection title="Bank Statements" data={bankStatements} handleView={handleView} />
        </div>
    );
}
